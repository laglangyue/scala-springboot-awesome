...
case class Users(userId: Int,
  ...
  def save()(implicit session: DBSession = Users.autoSession): Users =
    Users.save(this)(session)

  def destroy()(implicit session: DBSession = Users.autoSession): Int =
    Users.destroy(this)(session)
...
object Users extends SQLSyntaxSupport[Users] {
  ...
  def apply(u: SyntaxProvider[Users])(rs: WrappedResultSet): Users =
  ...
  def find(
    userId: Int
  )(implicit session: DBSession = autoSession): Option[Users] = {
  ...
  def findAll()(implicit session: DBSession = autoSession): List[Users] = {
    sql"""select ${u.result.*} from ${Users as u}"""
      .map(Users(u.resultName))
      .list
      .apply()
  }
  ...
  def countAll()(implicit session: DBSession = autoSession): Long = {
    sql"""select count(1) from ${Users.table}"""
      .map(rs => rs.long(1))
      .single
      .apply()
      .get
  ...
  def findAllBy(
    where: SQLSyntax
  )(implicit session: DBSession = autoSession): List[Users] = {
    sql"""select ${u.result.*} from ${Users as u} where ${where}"""
      .map(Users(u.resultName))
      .list
      .apply()
  ...
  def batchInsert(entities: collection.Seq[Users])(implicit session: DBSession =
                                                     autoSession): List[Int] = {
    val params: collection.Seq[Seq[(String, Any)]] = entities.map(
      entity =>
        Seq(
  ...
  def destroy(entity: Users)(implicit session: DBSession = autoSession): Int = {
    sql"""delete from ${Users.table} where ${column.userId} = ${entity.userId}""".update
      .apply()
  }
